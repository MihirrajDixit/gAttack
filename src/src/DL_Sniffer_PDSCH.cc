...
int PDSCH_Decoder::decode_dl_mode()
{
	...
	for (auto decoding_mem : (*ran_dl_collection))
	{
		if ((decoding_mem.ran_pdsch_grant->tb[0].tbs > 0 && decoding_mem.ran_dci_dl->rnti > 0 &&									 // only decode if packet length > 0 and rnti != 0,
			...
			(decoding_mem.rnti == 65534))																							 // always decode DCI with paging RNTI
		{
			if (decoding_mem.ran_dci_dl->tb[0].rv < 0)
			{
				// print_dl_grant_dci(*decoding_mem.ran_dci_dl.get(), tti, decoding_mem.rnti);
				if (decoding_mem.rnti == 65535)
				{ // on some base stations, SIB1 has DCI 1C with rv index = -1, change it to 0 to decode?
			...
			{ // If dont know MCS table, try both tables
			...
			// update statistic
			if (RNTI_name == "C_RNTI" && mcs_tracking_mode)
			{ // T_Note: changed for debuging && mimo_ret == SRSRAN_SUCCESS||| && decoding_mem.format > SRSRAN_DCI_FORMAT1A
